...
from pytorch3d.ops import packed_to_padded, eyes, sample_points_from_meshes
...
import torch
import sys
from im2mesh.common import arange_pixels, get_tensor_values
...
from pytorch3d.structures import Meshes, padded_to_list
from DSS.utils import convert_tensor_property_to_value_dict
from common import get_tri_color_lights, get_colored_lights
...
if __name__ == "__main__":
    ...
    for mesh_name, mesh_path in names_and_path:
        output_dir = os.path.join(opt.output, mesh_name)
        ...
        data_dict = {"cameras_type": '.'.join([camera_sampler.camera_type.__module__,
                     ...
                     "lights_type": '.'.join([lights.__module__, lights.__class__.__name__]),
                     }
        num_points = 10000
        ...
        data_dict['lights'] = convert_tensor_property_to_value_dict(lights)
        ...
        for c_idx, cams in enumerate(camera_sampler):
            ...
            for b in range(images.shape[0]):
                ...
                # temporary solution
                imageio.imwrite(os.path.join(depth_dir, "%06d.png" % idx),
                                dense_depths[b, ...].cpu())
                # save rgb
