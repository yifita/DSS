...
class LaplaceFilter(nn.Module):
    ...
    def __init__(self,
        ...
        # gaussian
        if k_gaussian > 1:
            gaussian_2D = self.get_gaussian_kernel(k_gaussian, sigma)
        ...
        # laplacian
        self.laplace_filter_weight = torch.tensor(self.get_laplacian_kernel()).to(
            dtype=torch.float, device=device).view(1, 1, 3, 3)
        self.laplace_filter_weight = torch.nn.Parameter(
            self.laplace_filter_weight, requires_grad=False)
        # thin
        self.nms = nms
        if nms:
    ...
    @classmethod
    def get_gaussian_kernel(cls, k=3, sigma=-1, normalize=True) -> np.array:
        """
    ...
    def _filter_with_gaussian(self, img):
        B, C, H, W = img.shape
        padding = tuple(
    ...
    def _filter_with_laplace(self, img):
        B, C, H, W = img.shape
        padding = (1, 1, 1, 1)
        img = F.pad(img, padding, mode='replicate')
        dst = F.conv2d(img.view(-1, 1, img.shape[2], img.shape[3]), self.laplace_filter_weight,
                       ).view(B, C, H, W)
        return dst
    ...
    def forward(self, img, hysteresis=False):
        ...
        B, C, H, W = img.shape
        # gaussian
        if self.gaussian_filter_weight is not None:
            blurred = self._filter_with_gaussian(img)
        else:
            blurred = img

        laplace = self._filter_with_laplace(blurred)

        output = laplace_abs = laplace.abs()
        ...
        if self.nms:
            ...
            laplace_abs_padded = F.pad(
                laplace_abs, padding, mode='replicate')
            directional = F.conv2d(laplace_abs_padded, self.directional_filter_weight,
                                   )
            # get indices of positive and negative directions
            positive_idx = (grad_orientation / 45) % 8
            negative_idx = ((grad_orientation / 45) + 4) % 8
            thin_edges = laplace_abs.clone()
...
class ImageSaliencySampler(nn.Module):
    ...
    def __init__(self, k_gaussian=3,
                 sigma=-1,
                 nms=False,
                 patch_size=1,
                 jitter=False,
        ...
        """
        super().__init__()
        self.edge_extractor = LaplaceFilter(k_gaussian=k_gaussian,
                                            sigma=sigma,
                                            nms=nms,
                                            thresholding=False,
                                            device=device)
    ...
    def get_edge_img(self, image, mask, alpha=1e1):
        """
        Args:
            image: (N,3,H,W)
            mask: (N,1,H,W)
        returns edge image (N,1,H,W)
        """
        # image gradient
        # 0. downsample image to 128x128
        batch_size = image.shape[0]
        # to gray image
        image = (0.2989 * image[:, 0, ...] + 0.5870 * image[:,
                                                            1, ...] + 0.1140 * image[:, 2, ...]).unsqueeze(1)

        H = min(image.shape[2], 128)
        W = min(image.shape[3], 128)
        image_small = F.interpolate(
            image, (H, W), mode='bilinear', align_corners=False)
        ...
        if mask is not None:
            ...
            mask_small = F.interpolate(
                mask.float(), (H, W), mode='nearest').bool()
            mask_sihlouette = self.edge_extractor(mask_small.float())
            thin_edge[(mask_sihlouette > 0.01) & (thin_edge > 0)] = 0
            thin_edge[~mask_small] = 0

        thin_edge = torch.tanh(thin_edge * alpha)
        return thin_edge

    def forward(self, n_pixels: int, image: torch.Tensor, mask: torch.Tensor = None, patch_size: Optional[int] = None,
                jitter: Optional[bool] = None, verbose=False):
        """
        ...
        thin_edge = self.get_edge_img(image, mask)
        H, W = thin_edge.shape[2:4]
        ...
        # to [-1~1, -1~1]
        xy_small[0] = (xy_small[0] * 2.0 / (W - 1) - 1.0)
        xy_small[1] = (xy_small[1] * 2.0 / (H - 1) - 1.0)
        xy_small = xy_small.permute([1, 2, 0]).contiguous()

        # add some small noise to the sample position, variance 2 pixel-size
        if jitter:
            xy_small += torch.randn_like(xy_small).sign() * \
                4 * min(2.0 / image.shape[2], 2.0 / image.shape[3])
        ...
        w_step = 2. / (image.shape[3] - 1)
        h_step = 2. / (image.shape[2] - 1)
        ...
        if verbose:
            return thin_edge, p.view(batch_size, -1, 2)
