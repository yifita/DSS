...
namespace
{
    ...
    __device__ inline float eps_denom(const float denom, const float eps)
    {
        int denom_sign = (0.0f < denom) - (denom < 0.0f);
        float safe_denom = float(denom_sign) * max(abs(denom), eps);
        return safe_denom;
    }
...
// *                            BACKWARD PASS                                 *
// ****************************************************************************
__global__ void RasterizePointsWeightsBackwardCudaKernel(
    const float *points,         // (P, 3)
    const float *ellipse_params, // (P, 3)
    const at::PackedTensorAccessor<float, 1> cutoff,
    ...
    const float *grad_qvalues, // (N, H, W, K)
    float *grad_points)
{ // (P, 3)
    ...
    for (int i = tid; i < N * H * W * K; i += num_threads)
    {
        ...
        if (p < 0)
            continue;
        const float grad_qvalue_pix = grad_qvalues[i];
        const int p_ind = p * 3; // index into packed points tensor
        const float px = points[p_ind + 0];
        const float py = points[p_ind + 1];
        const float a = ellipse_params[p_ind + 0];
        const float b = ellipse_params[p_ind + 1];
        const float c = ellipse_params[p_ind + 2];
        const float dx = xf - px;
        const float dy = yf - py;
        const float grad_px = -(2.0f * a * dx + b * dy) * grad_qvalue_pix;
        const float grad_py = -(2.0f * c * dy + b * dx) * grad_qvalue_pix;
        const float grad_pz = grad_zbuf[i];
        gpuAtomicAdd(grad_points + p_ind + 0, grad_px);
        gpuAtomicAdd(grad_points + p_ind + 1, grad_py);
        gpuAtomicAdd(grad_points + p_ind + 2, grad_pz);
    }
}

/* pass gradients only from the nonzero weights */
std::tuple<at::Tensor, at::Tensor> RasterizePointsWeightsBackwardCuda(
    const at::Tensor &points,         // (P, 3)
    const at::Tensor &ellipse_params, // (P, 3)
    const at::Tensor &cutoff,         // (P,) or (1,)
    const at::Tensor &idxs,           // (N, H, W, K)
    const at::Tensor &grad_zbuf,      // (N, H, W, K)
    const at::Tensor &grad_qvalues)
{ // (N, H, W, K)
    ...
    at::TensorArg points_t{points, "points", 1},
        ellipse_params_t{ellipse_params, "ellipse_params", 2},
        idxs_t{idxs, "idxs", 3},
        grad_zbuf_t{grad_zbuf, "grad_zbuf", 4},
        grad_qvalues_t{grad_qvalues, "grad_qvalues", 5};
    at::CheckedFrom c = "RasterizePointsWeightsBackward";
    at::checkAllSameGPU(c, {points_t, idxs_t, grad_zbuf_t, grad_qvalues_t});
    at::checkAllSameType(c, {points_t, grad_zbuf_t, grad_qvalues_t});
    ...
    at::Tensor grad_cutoff = at::zeros_like(cutoff);

    if (grad_points.numel() == 0)
    {
        AT_CUDA_CHECK(cudaGetLastError());
        return std::make_tuple(grad_points, grad_cutoff);
    }

    const size_t blocks = 1024;
    const size_t threads = 64;

    RasterizePointsWeightsBackwardCudaKernel<<<blocks, threads, 0, stream>>>(
        points.contiguous().data_ptr<float>(),
        ellipse_params.contiguous().data_ptr<float>(),
        cutoff.packed_accessor64<float, 1>(),
        ...
        grad_qvalues.contiguous().data_ptr<float>(),
...
/* Naive RasterizePointsOccRBFBackwardCudaKernel */
__global__ void RasterizePointsOccRBFBackwardCudaKernel(
    const float *points,         // (P, 3)
    const float *ellipse_params, // (P, 3)
    const at::PackedTensorAccessor64<float, 1> cutoff_thres,
    const float *radii,                       // (P, 2)
    const int32_t *idxs,                      // (N,H,W,3)
    const float *zbuf0,                       // (N,H,W)
    const int64_t *cloud_to_packed_first_idx, // (N)
    const int64_t *num_points_per_cloud,      // (N)
    const float radii_s,
    const float depth_merging_thres,
    ...
    const int S,
    const int K,
    const float *grad_occ,  // (N, S, S)
    const float *grad_zbuf, // (N, S, S, K)
    ...
    for (int i = tid; i < N * S * S; i += num_threads)
    {
        ...
        if (grad_occ_pix != 0.0f)
        {
            ...
            for (int p_idx = point_start_idx; p_idx < point_stop_idx; ++p_idx)
            {
                ...
                if (pixz > 0 && (pz - pixz) > depth_merging_thres)
                    continue;
                if (qvalue_pix > cutoff_thres_scaled)
                    continue;
                // point is not on the same surface
                const float expQ = exp(-0.5 * qvalue_pix);
                const float grad_px = 0.5 * expQ * (2 * a * dx + b * dy) * grad_occ_pix;
                const float grad_py = 0.5 * expQ * (2 * c * dy + b * dx) * grad_occ_pix;
                // printf("grad_pts[%d] = [%g, %g]\n", p_idx, grad_px, grad_py);
                gpuAtomicAdd(grad_points + p_idx * 3 + 0, grad_px);
                gpuAtomicAdd(grad_points + p_idx * 3 + 1, grad_py);
            }
        }
        ...
        for (int k = 0; k < K; k++)
        {
            ...
            gpuAtomicAdd(grad_points + p_idx * 3 + 2, grad_pz);
        }
    }
}

std::tuple<at::Tensor, at::Tensor> RasterizePointsOccRBFBackwardCuda(
    const at::Tensor &points,         // (P, 3)
    const at::Tensor &ellipse_params, // (P, 3)
    const at::Tensor &cutoff_thres,
    const at::Tensor &radii,     // (P, 2)
    const at::Tensor &idx,       // (N, H, W, K)
    const at::Tensor &zbuf0,     // (N, H, W)
    ...
    const float depth_merging_thres)
{
    // Check inputs are on the same device
    at::TensorArg points_t{points, "points", 1},
        ellipse_params_t{ellipse_params, "ellipse_params", 3},
    ...
    at::CheckedFrom c = "RasterizePointsOccRBFBackwardCuda";
    at::checkAllSameGPU(
        c, {points_t, radii_t, ellipse_params_t, cutoff_thres_t, idx_t, zbuf0_t, grad_occ_t, grad_zbuf_t,
    ...
    const int P = points.size(0);
    const int N = zbuf0.size(0);
    const int H = zbuf0.size(1);
    const int W = zbuf0.size(2);
    const int K = idx.size(3);

    const int S = H;
    at::Tensor grad_points = at::zeros({P, 3}, points.options());
    at::Tensor grad_cutoff = at::zeros_like(cutoff_thres);
    ...
    const size_t blocks = 1024;
    const size_t threads = 64;
    RasterizePointsOccRBFBackwardCudaKernel<<<blocks, threads, 0, stream>>>(
        points.contiguous().data_ptr<float>(),
        ellipse_params.contiguous().data_ptr<float>(),
        cutoff_thres.packed_accessor64<float, 1>(),
        radii.contiguous().data_ptr<float>(),
        idx.contiguous().data_ptr<int32_t>(),
        zbuf0.contiguous().data_ptr<float>(),
        cloud_to_packed_first_idx.contiguous().data_ptr<int64_t>(),
        num_points_per_cloud.contiguous().data_ptr<int64_t>(),
        radii_s,
        depth_merging_thres,
        ...
        grad_zbuf.contiguous().data_ptr<float>(),
        grad_points.contiguous().data_ptr<float>(),
        grad_cutoff.contiguous().data_ptr<float>());
    ...
    for (int i = tid; i < N * S * S; i += num_threads)
    {
        ...
        if (grad_occ_pix != 0.0f)
        {
            ...
            for (int p_idx = point_start_idx; p_idx < point_stop_idx; ++p_idx)
            {
                ...
                // pixel outside the splat and is rendered yet it should be
                ...
                if (abs(dx) > radiix && abs(dy) > radiiy)
                    continue; // Skip if pixel out of precomputed radii range
                if (pixz > 0 && (pz - pixz) > depth_merging_thres)
                    continue; // skip if point doesn't belong to the same (frontal) surface
...
std::tuple<at::Tensor, at::Tensor> RasterizePointsOccBackwardCuda(
    const at::Tensor &points,         // (P, 3)
    const at::Tensor &ellipse_params, // (P, 3)
    const at::Tensor &cutoff_thres,
    const at::Tensor &radii,     // (P, 2)
    ...
    const at::Tensor &zbuf0,     // (N, H, W)
    const at::Tensor &grad_occ,  //  (N, H, W)
    const at::Tensor &grad_zbuf, // (N, H, W)
    const at::Tensor &cloud_to_packed_first_idx,
    const at::Tensor &num_points_per_cloud,
    const float radii_s,
    const float depth_merging_thres)
{
    ...
    at::checkAllSameGPU(
        c, {points_t, radii_t, ellipse_params_t, idx_t, zbuf0_t, grad_occ_t, grad_zbuf_t,
            cloud_to_packed_first_idx_t, num_points_per_cloud_t});
