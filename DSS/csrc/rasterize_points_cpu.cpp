...
float Qvalue(const float dx, const float dy, const float a, const float b, const float c)
{
  return a * dx * dx + b * dx * dy + c * dy * dy;
}

float SigmoidDiff(const float x)
{
  if (abs(x) > 12)
    return 0.0f;
...
Args:
  ...
  TODO(yifan) another condition Q < radii_s^2 * cutoff ?
 */
    ...
    const float depth_merging_thres)
{
  ...
  torch::Tensor grad_cutoff = torch::zeros_like(cutoff_thres);
  ...
  auto ellipse_params_a = ellipse_params.accessor<float, 2>();
  auto cutoff_a = cutoff_thres.accessor<float, 1>();
  auto zbuf0_a = zbuf0.accessor<float, 3>();
  auto idxs_a = idx.accessor<int32_t, 4>();
  // outputs
  auto grad_points_a = grad_points.accessor<float, 2>();
  auto grad_cutoff_a = grad_cutoff.accessor<float, 1>();
    ...
    for (int yi = 0; yi < S; ++yi)
    {
      ...
      for (int xi = 0; xi < S; ++xi)
      {
        ...
        { // Loop over points for the pixel
          ...
          if (p < 0)
            break;
          grad_points_a[p][2] += grad_zbuf_a[n][yi][xi][k];
        }
        ...
        for (int p = point_start_idx; p < point_stop_idx; ++p)
        {
          ...
          const float py = points_a[p][1];
          const float pz = points_a[p][2];
          const float cutoff_thres_scaled = cutoff_a[p] * radii_s * radii_s;
          ...
          // if > 0, it means this pixel shouldn't be occupied, but it's where the splat should
          // be (moving away from this pixel doesn't improve loss), so set the gradient to zero
          const bool pix_outside_splat = abs(dx) > radii_a[p][0] && abs(dy) > radii_a[p][1];
          ...
          if (abs(dx) > radiix && abs(dy) > radiiy)
            continue;
          // The current pixel is in the extended range of the point
          if (pixz > 0 && (pz - pixz) > depth_merging_thres)
            continue;
          // The point is on the same surface
          const float a = ellipse_params_a[p][0];
          const float b = ellipse_params_a[p][1];
          const float c = ellipse_params_a[p][2];
          const float qvalue_pix = a * dx * dx + b * dy * dx + c * dy * dy;
          if (qvalue_pix > cutoff_thres_scaled)
            continue;
          // weights proportional to exp(-0.5Q)
          grad_points_a[p][0] += 0.5 * exp(-0.5f * qvalue_pix) * (2 * a * dx + b * dy) * grad_occ_pix;
          grad_points_a[p][1] += 0.5 * exp(-0.5f * qvalue_pix) * (2 * c * dy + b * dx) * grad_occ_pix;

          // gradient for cutoff threshold
          const float cutoff_value = cutoff_a[p];
          float qdiff = cutoff_value - qvalue_pix;
          const float cutoff_grad_pix = 1e-4 * sgn(grad_occ_pix) * SigmoidDiff(-qdiff);
          grad_cutoff_a[p] += cutoff_grad_pix;
        }
      }
    }
  }
    ...
    for (int yi = 0; yi < S; ++yi)
    {
      ...
      for (int xi = 0; xi < S; ++xi)
      {
        ...
        for (int p = point_start_idx; p < point_stop_idx; ++p)
        {
          ...
          const float b = ellipse_params_a[p][1];
          const float c = ellipse_params_a[p][2];
          const float qvalue_pix = Qvalue(dx, dy, a, b, c);
          ...
          // if > 0, it means this pixel shouldn't be occupied, but it's where the splat should
          // be (moving away from this pixel doesn't improve loss), so set the gradient to zero
          const bool pix_outside_splat = qvalue_pix > cutoff_a[p];
          if (grad_occ_pix > 0.0f && pix_outside_splat)
            continue;
          ...
          const float denom = std::max(dist2, 1e-10f);

          grad_points_a[p][0] += 1.0f / denom * dx * grad_occ_pix;
          grad_points_a[p][1] += 1.0f / denom * dy * grad_occ_pix;
        }
      }
    }
  }
...
std::tuple<torch::Tensor, torch::Tensor> RasterizePointsWeightsBackwardCpu(
    ...
    const torch::Tensor &grad_zbuf,      // (N, H, W, K)
    const torch::Tensor &grad_qvalues    // (P, 3)
)
{
  ...
  torch::Tensor grad_points = torch::zeros({P, 3}, points.options());
  torch::Tensor grad_cutoff = torch::zeros_like(cutoff);

  auto points_a = points.accessor<float, 2>();
  auto idxs_a = idxs.accessor<int32_t, 4>();
  auto ellipse_params_a = ellipse_params.accessor<float, 2>();
  auto grad_qvalues_a = grad_qvalues.accessor<float, 4>();
  auto cutoff_a = cutoff.accessor<float, 1>();
  auto grad_zbuf_a = grad_zbuf.accessor<float, 4>();
  auto grad_points_a = grad_points.accessor<float, 2>();
  auto grad_cutoff_a = grad_cutoff.accessor<float, 1>();
    ...
    { // Loop over rows in the image
      ...
      { // Loop over pixels in the row
        ...
        { // Loop over points for the pixel
          ...
          const float grad_qvalue_pix = grad_qvalues_a[n][y][x][k];
          const float px = points_a[p][0];
          const float py = points_a[p][1];
          const float dx = xf - px;
          const float dy = yf - py;
          ...
          // if (grad_qvalue_pix != 0)
          // {
          //   printf("grad_p = (%g, %g)\n", grad_px, grad_py);
          // }
          const float qvalue = Qvalue(dx, dy, a, b, c);
          const float cutoff_value = cutoff_a[p];
          const float qdiff = 10 * (cutoff_value - qvalue);
          grad_cutoff_a[p] += grad_qvalue_pix * exp(-qdiff) / (1 + exp(-qdiff)) * (1 + exp(-qdiff));
