...
template <typename T>
__device__ inline T eps_denom(const T denom, const T eps)
{
    int denom_sign = (T(0.0) < denom) - (denom < T(0.0));
    T safe_denom = T(denom_sign) * max(abs(denom), eps);
    return safe_denom;
}
template <typename T>
__device__ inline T clamp(const T x, const T a, const T b)
{
